...
<article id="music-{i}" onclick={setCurrentTrack} style={currentTrack.path === track.path ? `background-color: ${currentColor.background}; color: ${currentColor.text};` : ''} class="select-none grid cursor-pointer grid-cols-12 items-center gap-2 rounded-md py-3 px-4 snap-start text-xs md:text-sm lg:text-base {currentTrack.path === track.path ? '' : 'text-white hover:bg-gray-800'}">
